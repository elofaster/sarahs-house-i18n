<p align="center"><img src="docs/assets/readme-header.png" alt="Sarah's House — Translation Mod" width="100%"></p>

[English](README.md) · [Русский](README.ru.md) · [Українська](README.uk.md) · [Deutsch](README.de.md) · [Español](README.es.md) · [Français](README.fr.md) · [Português](README.pt.md) · [Türkçe](README.tr.md) · [Polski](README.pl.md) · [Tiếng Việt](README.vi.md) · [中文](README.zh.md) · [日本語](README.ja.md) · [한국어](README.ko.md)

# Sarah's House — Translation Mod

『Sarah's House』の非公式多言語 Mod（Windows、v0.11.6）。

- ウェブサイト: https://elofaster.github.io/sarahs-house-i18n/
- ダウンロード: https://github.com/elofaster/sarahs-house-i18n/releases/download/v2.9.1/SarahsHouse-i18n-v2.9.1.zip
- 最新リリース: https://github.com/elofaster/sarahs-house-i18n/releases/latest

## ゲームを応援する

『Sarah's House』は AceStudio による有料ゲームで、この Mod は翻訳のみを提供します。ゲームを気に入ったら、購入して開発者を応援してください：

- 🛒 Steam — https://store.steampowered.com/app/4712060/Sarahs_House
- 🎮 itch.io — https://ace-stud.itch.io/sarahs-house

## 言語

| | コード | 言語 |
|---|---|---|
| 🇬🇧 | EN | English（オリジナル） |
| 🇷🇺 | RU | Русский |
| 🇺🇦 | UK | Українська |
| 🇩🇪 | DE | Deutsch |
| 🇪🇸 | ES | Español |
| 🇫🇷 | FR | Français |
| 🇵🇹 | PT | Português |
| 🇹🇷 | TR | Türkçe |
| 🇵🇱 | PL | Polski |
| 🇻🇳 | VI | Tiếng Việt |
| 🇨🇳 | ZH | 中文 |
| 🇯🇵 | JA | 日本語 |
| 🇰🇷 | KO | 한국어 |

## インストール

1. リリースのアーカイブを、`SarahsHouse.exe` と同じゲームフォルダーに展開します。
2. ゲームを起動します。初回起動は準備のため 1〜3 分かかることがあります。
3. ゲーム内のパネルで言語を選びます：`F10` キー（または右上の言語アイコン）。

## リポジトリ

- `docs/` — ウェブサイト（GitHub Pages）
- `mod/` — Mod 本体：プラグイン、言語パック、フォント

---

非公式のファン Mod です。AceStudio とは関係ありません。