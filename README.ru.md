<p align="center"><img src="docs/assets/readme-header.png" alt="Sarah's House — Translation Mod" width="100%"></p>

[English](README.md) · [Русский](README.ru.md) · [Українська](README.uk.md) · [Deutsch](README.de.md) · [Español](README.es.md) · [Français](README.fr.md) · [Português](README.pt.md) · [Türkçe](README.tr.md) · [Polski](README.pl.md) · [中文](README.zh.md) · [日本語](README.ja.md) · [한국어](README.ko.md)

# Sarah's House — Translation Mod

Неофициальный мультиязычный мод для Sarah's House (Windows, v0.11.2).

- Сайт: https://elofaster.github.io/sarahs-house-i18n/
- Скачать: https://github.com/elofaster/sarahs-house-i18n/releases/download/v2.9.0/SarahsHouse-i18n-v2.9.0.zip
- Последний релиз: https://github.com/elofaster/sarahs-house-i18n/releases/latest

## Поддержите игру

Sarah's House — платная игра от студии AceStudio. Этот мод лишь переводит её. Если игра вам нравится — купите её и поддержите разработчиков:

- 🛒 Steam — https://store.steampowered.com/app/4712060/Sarahs_House
- 🎮 itch.io — https://ace-stud.itch.io/sarahs-house

## Языки

| | Код | Язык |
|---|---|---|
| 🇬🇧 | EN | English (оригинал) |
| 🇷🇺 | RU | Русский |
| 🇺🇦 | UK | Українська |
| 🇩🇪 | DE | Deutsch |
| 🇪🇸 | ES | Español |
| 🇫🇷 | FR | Français |
| 🇵🇹 | PT | Português |
| 🇹🇷 | TR | Türkçe |
| 🇵🇱 | PL | Polski |
| 🇨🇳 | ZH | 中文 |
| 🇯🇵 | JA | 日本語 |
| 🇰🇷 | KO | 한국어 |

## Установка

1. Распакуйте архив релиза в папку с игрой, рядом с `SarahsHouse.exe`.
2. Запустите игру — первый запуск может занять 1–3 минуты на подготовку.
3. Выберите язык в панели прямо в игре: клавиша `F10` (или значок языка в правом верхнем углу).

## Репозиторий

- `docs/` — сайт (GitHub Pages)
- `mod/` — сам мод — плагин, языковые пакеты и шрифты

---

Неофициальный фанатский мод. Не связан с AceStudio.
