<p align="center"><img src="docs/assets/readme-header.png" alt="Sarah's House — Translation Mod" width="100%"></p>

[English](README.md) · [Русский](README.ru.md) · [Українська](README.uk.md) · [Deutsch](README.de.md) · [Español](README.es.md) · [Français](README.fr.md) · [Português](README.pt.md) · [Türkçe](README.tr.md) · [Polski](README.pl.md) · [Tiếng Việt](README.vi.md) · [中文](README.zh.md) · [日本語](README.ja.md) · [한국어](README.ko.md)

# Sarah's House — Translation Mod

Sarah's House için resmi olmayan çok dilli bir mod (Windows, v0.11.6).

- Web sitesi: https://elofaster.github.io/sarahs-house-i18n/
- İndir: https://github.com/elofaster/sarahs-house-i18n/releases/download/v2.9.1/SarahsHouse-i18n-v2.9.1.zip
- Son sürüm: https://github.com/elofaster/sarahs-house-i18n/releases/latest

## Oyunu destekleyin

Sarah's House, AceStudio'nun ücretli bir oyunudur. Bu mod yalnızca onu çevirir — oyunu beğendiyseniz, satın alarak geliştiricileri destekleyin:

- 🛒 Steam — https://store.steampowered.com/app/4712060/Sarahs_House
- 🎮 itch.io — https://ace-stud.itch.io/sarahs-house

## Diller

| | Kod | Dil |
|---|---|---|
| 🇬🇧 | EN | English (orijinal) |
| 🇷🇺 | RU | Русский |
| 🇺🇦 | UK | Українська |
| 🇩🇪 | DE | Deutsch |
| 🇪🇸 | ES | Español |
| 🇫🇷 | FR | Français |
| 🇵🇹 | PT | Português |
| 🇹🇷 | TR | Türkçe |
| 🇵🇱 | PL | Polski |
| 🇻🇳 | VI | Tiếng Việt |
| 🇨🇳 | ZH | 中文 |
| 🇯🇵 | JA | 日本語 |
| 🇰🇷 | KO | 한국어 |

## Kurulum

1. Sürüm arşivini oyun klasörüne, `SarahsHouse.exe` dosyasının yanına çıkarın.
2. Oyunu başlatın: ilk açılış, hazırlık için 1–3 dakika sürebilir.
3. Dilinizi oyun içindeki panelden seçin: `F10` tuşu (veya sağ üst köşedeki dil simgesi).

## Depo

- `docs/` — web sitesi (GitHub Pages)
- `mod/` — modun kendisi: eklenti, dil paketleri ve yazı tipleri

---

Resmi olmayan bir hayran modu. AceStudio ile bağlantılı değildir.